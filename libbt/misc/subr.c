...
static int dir_make_parent(char *path)
{
    ...
    char r_dir[MAX_PATH];
    char *dir = NULL;
    memset(r_dir,0,sizeof(r_dir));

    while(dir = strtok(path,"/"))
    {
	sprintf(r_dir+strlen(r_dir),"%s/",dir);
	if(dir[strlen(dir) -1] == ':')
	{
	    path = NULL;
	    continue;
	}
	rv = CreateDirectoryA(r_dir,NULL);
...
mkdirs(char *path, int mode)
{
    ...
    for(p = dirs + len; len > 0 && (*p != '/'); --len,--p);
    if(len > 0)
    {
	*p = '\0';
    }
    dir_make_parent(dirs);
    return 0;
}
...
vaopen(int *res, int flags, const char *fmt, va_list ap)
{
    ...
    if (vsnprintf(path, MAX_PATH, fmt, ap) >= MAX_PATH)
        return ENAMETOOLONG;

    didmkdirs = 0;
again:
    fd = _open(path, flags|O_BINARY, 0666);
...
int vopen(int *res, int flags, const char *fmt,...)
{
    int err;
    va_list ap;
    va_start(ap,fmt);
...
int vtruncate(const char *path,fpos_t length)
{
    int fd,didmkdirs;
    int flags = O_RDWR|_O_BINARY;
    didmkdirs = 0;
again:
    fd = _open(path, flags, 0666);
    if(fd < 0 && errno == ENOENT && (flags & O_CREAT) != 0 && !didmkdirs) {
	char *rs = strchr(path,'/');
	if(rs != NULL) {
	    *rs = '\0';
	    if(mkdirs((char *)path, 0777) == 0) {
		*rs = '/';
		didmkdirs = 1;
		goto again;
	    }
...
vfsync(const char *fmt, ...)
{
    ...
    va_list ap;
    va_start(ap, fmt);
    err = vaopen(&fd, O_RDONLY|_O_BINARY, fmt, ap);
    va_end(ap);
    if (err != 0)
        return err;
//    if (fsync(fd) < 0)
...
{
    return min + (long)ceil((double)rand() * (max - min) / RAND_MAX);
}

int
write_fully(SOCKET fd, const void *buf, size_t len)
{
    ssize_t nw;
    size_t off = 0;
	DWORD code = 0;
...
int
read_fully(SOCKET fd, void *buf, size_t len)
{
    ssize_t nread;
    size_t off = 0;

    while (off < len) {
        nread = recv(fd, (char *)buf + off, len - off,0);
        if (nread == 0)
            return EIO;
        else if (nread == -1)
            return -1;
...
int f_write_fully(int fd,const void *buf,size_t len)
{
    DWORD off = 0;
    DWORD bytes = 0;
    BOOL bWrite;
...
int f_read_fully(int fd,void *buf, size_t len)
{
    DWORD off = 0;
    DWORD bytes = 0;
    BOOL bRead;
...
read_file(const char *path, void *buf, size_t *size)
{
    ...
    if ((fd = open(path, O_RDONLY|_O_BINARY)) == -1)
        return NULL;
    if (fstat(fd, &sb) == -1)
        goto error;
    ...
    *size = sb.st_size;
    ...
    if (buf == NULL)
        buf = mem;
    if ((errno = f_read_fully(fd, buf, *size)) != 0)
        goto error;
    _close(fd);
    return buf;
...
make_abs_path(const char *in, char *out)
{
    ...
    case '/':
        ...
        break;
    default:
        if (!GetCurrentDirectoryA(MAX_PATH,out))
            return errno;
        oi = strlen(out);
        if (oi + strlen(in) + 1 >= MAX_PATH)
            return ENAMETOOLONG;
        out[oi] = '/';
        lastsep = oi;
        oi++;
