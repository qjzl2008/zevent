...
int
bts_seek(struct bt_stream *bts, fpos_t off)
{
    unsigned i;
    if (bts->t_off == off)
        return 0;
...
int
bts_get(struct bt_stream *bts, fpos_t off, uint8_t *buf, size_t len)
{
    ...
    while (boff < len) {
        if (bts->fd == -1) {
            ...
            if (err != 0)
                return err;
            if (bts->f_off != 0)
                _lseeki64(bts->fd, bts->f_off, SEEK_SET);
        }

        wantread = min(len - boff, bts->files[bts->index].length - bts->f_off);
        err = f_read_fully(bts->fd, buf + boff, wantread);
	didread = wantread;
        if (err != 0)
            return err;
...
int
bts_put(struct bt_stream *bts, fpos_t off, const uint8_t *buf, size_t len)
{
    ...
    while (boff < len) {
        ...
        didwrite = _write(bts->fd, buf + boff, wantwrite);
        if (didwrite == -1)
            return errno;
...
int
bts_sha(struct bt_stream *bts, fpos_t start, fpos_t length, uint8_t *hash)
{
    SHA_CTX ctx;
    char buf[SHAFILEBUF];
    size_t wantread;
