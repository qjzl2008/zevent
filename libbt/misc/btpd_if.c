...
int
ipc_open(const char *ip, int port, struct ipc **out)
{
    SOCKET sd = -1;
    int err = 0;
    size_t plen;
...
enum ipc_err btpd_add_p2sp(struct ipc *ipc,struct ipc_torrent *tp,const char *url)
{
    struct iobuf iob = iobuf_init(1<<10);
    iobuf_print(&iob,"l7:addp2spd3:url%d:%s4:hash20:",(int)strlen(url),
	    url);
