...
tlib_del(struct tlib *tl)
{
    ...
    sprintf(path, "torrents/%s", bin2hex(tl->hash, relpath, 20));
    if ((dir = opendir(path)) != NULL) {
        while ((de = readdir(dir)) != NULL) {
            if (strcmp(".", de->d_name) == 0 || strcmp("..", de->d_name) == 0)
                continue;
            sprintf(path, "torrents/%s/%s", relpath, de->d_name);
            rv = remove(path);
        }
        closedir(dir);
    }
    sprintf(path, "torrents/%s", relpath);
    //remove(path);
...
{
    FILE *fp;
    char relpath[SHAHEXSIZE], path[MAX_PATH], wpath[MAX_PATH];
    ...
    sprintf(path, "torrents/%s/info", relpath);
    sprintf(wpath, "%s.write", path);

    if ((fp = fopen(wpath, "wb")) == NULL)
    ...
    const char *content, char *name)
{
    ...
    char relpath[RELPATH_SIZE], file[MAX_PATH];
    ...
    tl->name = name;
    tl->dir = _strdup(content);
    if (tl->name == NULL || tl->dir == NULL)
        btpd_err("out of memory.\r\n");

    sprintf(file, "torrents/%s", relpath);
    if (!CreateDirectoryA(file, NULL))
        btpd_err("failed to create dir '%s' (%s).\r\n", file, strerror(errno));
    sprintf(file, "torrents/%s/torrent", relpath);
...
tlib_init(void)
{
    ...
    {
	if(CreateDirectoryA("torrents",NULL))
	{
	    if((dirp = opendir("torrents")) == NULL)
	    {
    ...
    while ((dp = readdir(dirp)) != NULL) {
        if (strlen(dp->d_name) == 40 && ishex(dp->d_name)) {
            ...
            sprintf(file, "torrents/%s/info", dp->d_name);
            load_info(tl, file);
        }
    }
    closedir(dirp);
...
tlib_read_hash(struct tlib *tl, size_t off, uint32_t piece, uint8_t *hash)
{
    ...
    if ((errno = vopen(&fd, O_RDONLY|_O_BINARY, "torrents/%s/torrent", relpath)) != 0)
        btpd_err("failed to open 'torrents/%s/torrent' (%s).\r\n",
		relpath, strerror(errno));
    _lseek(fd, off + piece * 20, SEEK_SET);
    ret = f_read_fully(fd,hash,20);
    if (ret != 0){
	_close(fd);
...
{
    return (char *)resd->base + 8 + 16 * i;
}

static void *
resume_file_time(struct resume_data *resd, int i)
{
    return (char *)resd->base + 16 + 16 * i;
}
...
init_resume(int fd, size_t size)
{
    ...
    enc_be32(&ver, 2);
    if (_write(fd, "RESD", 4) == -1 || _write(fd, &ver, 4) == -1)
        goto fatal;
    size -= 8;
    while (size > 0) {
        ssize_t nw = _write(fd, buf, min(sizeof(buf), size));
        if (nw < 1)
            goto fatal;
        size -= nw;
    }
    ...
    size_t bfsize)
{
    ...
    if ((errno =
            vopen(&fd, O_RDWR|O_CREAT|_O_BINARY, "torrents/%s/resume", relpath)) != 0)
        goto fatal;
    if (fstat(fd, &sb) != 0)
        goto fatal;
    if (sb.st_size != resd->size) {
        if (sb.st_size != 0 && vftruncate(fd, 0) != 0)
            goto fatal;
        init_resume(fd, resd->size);
    }

    resd->maphandle = CreateFileMapping((HANDLE)_get_osfhandle(fd),NULL,PAGE_READWRITE,
	    0,0,NULL);
    if(resd->maphandle == INVALID_HANDLE_VALUE)
    {
	goto fatal;
    }
    resd->base = MapViewOfFile(resd->maphandle,FILE_MAP_WRITE | FILE_MAP_READ,0,0,
	    resd->size);
    ...
    if (resd->base == NULL)
        goto fatal;
    if (memcmp(resd->base, "RESD", 4) != 0 || dec_be32((char*)resd->base + 4) != 2)
        init_resume(fd, resd->size);
    _close(fd);

    resd->pc_field = (char *)resd->base + 8 + nfiles * 16;
    resd->blk_field = (char *)resd->pc_field + pfsize;
...
tlib_close_resume(struct resume_data *resd)
{
    ...
    UnmapViewOfFile(resd->base);
    CloseHandle(resd->maphandle);
    free(resd);
}
