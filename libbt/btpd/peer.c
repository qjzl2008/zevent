...
mp_drop(struct meta_peer *mp, struct net *n)
{
	...
	{
	    struct meta_peer *peer = mptbl_remove(n->mptbl,mp->id);
            assert(peer == mp);
	}
        mp_kill(mp);
    }
...
static struct peer *
peer_create_common_http(SOCKET sd,struct http_url *url,int idx)
{
    struct peer *p = btpd_calloc(1,sizeof(*p));
    char buf[20];
    ...
    p->ptype = HTTP_PEER;
    strcpy(p->url.ip,url->host);
    p->url.port = url->port;
    strcpy(p->url.uri,url->uri);

    p->mp = mp_create();
    mp_hold(p->mp);
    p->mp->p = p;
    
    _snprintf(buf,sizeof(buf),"%u%u%d",timeGetTime(),inet_addr(p->url.ip),idx);
...
static void
p2s_connected_cb(SOCKET sd, short type, void *arg)
{
	...
	case EV_WRITE:
	    ...
	    p->in.msg_num = P2SP_RESP;
	    p->n = p->n;
	    p->mp->flags = PF_I_WANT;
	    p->piece_field = btpd_calloc(1,(int)ceil(p->n->tp->npieces / 8.0));
	    memset(p->piece_field,0xFF,(int)ceil(p->n->tp->npieces /8.0));
	    for(i = 0; i<p->n->tp->npieces; i++) {
		p->n->piece_count[i]++;
		p->npieces++;
		if(has_bit(p->piece_field,i) && !cm_has_piece(p->n->tp,i)) {
		    peer_want(p,i);
		}
	    }
	    ...
	    peer_set_in_state(p,HTTP_RESP,0);
	    dl_on_download(p);
	    break;
	default:
	    abort();
...
struct peer* peer_create_out_http(struct net *n,struct http_url *url,int idx)
{
    char *ip = url->host;
    ...
    p = peer_create_common_http(sd,url,idx);
    p->n = n;
    btpd_ev_new(&p->ioev,p->sd,EV_WRITE,p2s_connected_cb,p);
    return p;
}
...
peer_create_out_compact(struct net *n, int family, const char *compact)
{
    ...
    case AF_INET:
        ...
        addrlen = sizeof(*a4);
        memcpy(&a4->sin_addr.s_addr,compact, 4);
        memcpy(&a4->sin_port,compact + 4, 2);
        break;
    case AF_INET6:
        if (!net_ipv6)
            return;
        ...
        addrlen = sizeof(*a6);
        memcpy(&a6->sin6_addr,compact,16);
        memcpy(&a6->sin6_port,compact + 16, 2);
        break;
    default:
        abort();
    }
...
peer_on_bitfield(struct peer *p, const uint8_t *field)
{
    ...
    assert(p->npieces == 0);
    memcpy(p->piece_field,field, (size_t)ceil(p->n->tp->npieces / 8.0));
    for (i = 0; i < p->n->tp->npieces; i++) {
...
void 
peer_on_piece_p2sp(struct peer *p,uint32_t index,uint32_t begin,
	uint32_t length,const char *data)
{
    struct block_request *req;
    struct block_request *tmpreq;
    struct net *n;
    struct piece *pc;
    int find = 0;
    ...
    while(req)
    {
	if ((req->p->in.pc_index) == index && (req->p->in.pc_begin == begin)) {
	    btpd_log(BTPD_L_MSG, "received piece(%u,%u,%u) from %p \r\n",
		    index,begin,length,p);
	    assert(p->nreqs_out > 0);
	    p->nreqs_out--;
	    ...
	    tmpreq = req;
	    req = BTPDQ_NEXT(req,p_entry);

	    BTPDQ_REMOVE(&p->my_reqs, tmpreq, p_entry);

	    n = p->n;
	    pc = dl_find_piece(n,index);

	    BTPDQ_REMOVE(&pc->reqs, tmpreq, blk_entry);
	    nb_drop(tmpreq->msg);
	    free(tmpreq);
	    pc->nreqs--;
    ...
    if(find)
    {
	dl_on_block_p2sp(p,index,begin,length,data);
    }
    else
    {
	btpd_log(BTPD_L_MSG,"discarded piece(%u,%u,%u) from %p\n",
...
peer_on_tick(struct peer *p)
{
    if(p->ptype == HTTP_PEER)
    {
	if(btpd_seconds - p->t_wantwrite >= 10)
	    goto kill;
    }
    else
    {
...
peer_good_piece(struct peer *p, uint32_t index)
{
    ...
    p->suspicion = 0;
    if ((p->ptype == BT_PEER) && (p->mp->flags & PF_SUSPECT)) {
