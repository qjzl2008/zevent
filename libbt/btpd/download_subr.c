...
piece_log_hashes(struct piece *pc)
{
	...
	for (i = 0; i < pc->nblocks; i++) {
		bsize = torrent_block_size(tp, pc->index, pc->nblocks, i);
...
piece_log_good(struct piece *pc)
{
	...
	while (bad != NULL) {
		BTPDQ_FOREACH(r, &bad->records, entry) {
			int culprit = 0;
			unsigned i = 0;
			for (i = 0; i < pc->nblocks && !culprit; i++)
				if (has_bit(r->down_field, i) && (
					memcmp(&log->hashes[i*20], &bad->hashes[i*20], 20) != 0))
					culprit = 1;
			if (culprit)
...
dl_enter_endgame(struct net *n)
{
	struct peer *p;
	struct piece *pc;
	struct piece **pcs = malloc(sizeof(struct piece *) * n->npcs_busy);
	unsigned pi;
	...
	BTPDQ_FOREACH(p, &n->peers, p_entry) {
		...
		if (peer_leech_ok(p) && p->ptype == BT_PEER)
...
dl_on_piece_unfull(struct piece *pc)
{
	...
	while (p != NULL && !piece_full(pc)) {
		...
		if((p->ptype == HTTP_PEER) && peer_leech_ok(p) && peer_requestable(p,pc->index)
			&& (p->nreqs_out == 0))
			dl_piece_assign_requests(pc, p); // Cannot provoke end game here.
...
dl_unassign_requests(struct peer *p)
{
	while (p->nreqs_out > 0) {
		...
		int was_full;
		if(p->ptype == HTTP_PEER)
			pc = dl_find_piece(p->n,p->in.pc_index);
		else
			pc = dl_find_piece(p->n,nb_get_index(req->msg));

		was_full = piece_full(pc);
		...
		while (req != NULL) {
			...
			{
				blki = req->p->in.pc_begin / PIECE_BLOCKLEN;
				if(next)
					idx = next->p->in.pc_index;
			}
			else
			{
				blki = nb_get_begin(req->msg) / PIECE_BLOCKLEN;
				if(next)
					idx = nb_get_index(next->msg);
				// XXX: Needs to be looked at if we introduce snubbing.
				assert(has_bit(pc->down_field, blki));
			}
			clear_bit(pc->down_field, blki);
			...
			while (next && idx != pc->index)
			{
				next = BTPDQ_NEXT(next, p_entry);
				if(!next)
					break;
				if(next->p->ptype == HTTP_PEER)
					idx = next->p->in.pc_index;
				else
					idx = nb_get_index(next->msg);
			}
