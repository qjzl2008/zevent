...
fd_cb_rd(const char *path, int *fd, void *arg)
{
    ...
    return vopen(fd, O_RDONLY|_O_BINARY, "%s/%s", tp->tl->dir, path);
}
...
{
    struct torrent *tp = arg;
    return vopen(fd, O_RDWR|_O_BINARY, "%s/%s", tp->tl->dir, path);
}
...
cm_save(struct torrent *tp)
{
    ...
    int i = 0;
    struct file_time_size *fts = malloc(sizeof(struct file_time_size) * tp->nfiles);
...
cm_test_piece(struct torrent *tp, uint32_t piece)
{
    ...
    if ((errno = test_piece(tp, piece, &ok)) != 0)
        ...
        memset(cm->block_field + piece * cm->bppbf,0, cm->bppbf);
        if (net_active(tp))
    ...
    const uint8_t *buf, size_t len)
{
    ...
    bf = cm->block_field + piece * cm->bppbf;
    ...
    if (!has_bit(cm->pos_field, piece)) {
        ...
        while (start < end) {
            if (!has_bit(cm->pos_field, start)) {
                ...
                tmplen = torrent_piece_size(tp, start);
                off = tp->piece_length * start;
                while (tmplen > 0) {
                    wlen = min(ZEROBUFLEN, tmplen);
...
stat_and_adjust(struct torrent *tp, struct file_time_size ret[])
{
    ...
    int i = 0;
    for (i = 0; i < tp->nfiles; i++) {
        sprintf(path, "%s/%s", tp->tl->dir, tp->files[i].path);
again:
        if (stat(path, &sb) == -1) {
            if (errno == ENOENT) {
                errno = vopen(&fd, O_CREAT|O_RDWR|_O_BINARY, "%s", path);
                if (errno != 0 || _close(fd) != 0) {
            ...
            if (vtruncate(path, tp->files[i].length) != 0) {
...
{
    uint32_t piece = 0,*bf,nblocks,nblocks_got;
    uint32_t i;
    struct content *cm = tp->cm;

    memset(cm->pos_field,0, ceil(tp->npieces / 8.0));
    for (piece = 0; piece < tp->npieces; piece++) {
        ...
        bf = cm->block_field + cm->bppbf * piece;
        nblocks = torrent_piece_blocks(tp, piece);
        nblocks_got = 0;
        for (i = 0; i < nblocks; i++) {
            if (has_bit(bf, i)) {
                nblocks_got++;
        ...
        for (i = 0; i < tp->nfiles; i++) {
            if (fts[i].size != tp->files[i].length) {
                ...
                while (start <= end) {
                    ...
                    memset(cm->block_field + start * cm->bppbf,0, cm->bppbf);
                    start++;
                }
            }
