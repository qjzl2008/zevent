...
choke_do(void)
{
	...
	struct peer_sort *worthy = (struct peer_sort *)malloc(sizeof(struct peer_sort) *
		m_npeers);
        int nworthy = 0;
        int i = 0;
        int found = 0;
        struct peer *p;
	int *unchoked = (int *)malloc(sizeof(int) * m_npeers);
        ...
        memset(unchoked, 0, sizeof(int) * m_npeers);
...
choke_cb(int sd, short type, void *arg)
{
    ...
    struct timespec ts;
    ts.tv_sec = CHOKE_INTERVAL;
    ts.tv_nsec = 0;
    btpd_timer_add(&m_choke_timer, &ts);
    cb_count++;
    if (cb_count % 3 == 0)
        shuffle_optimists();
    choke_do();
