...
torrent_piece_size(struct torrent *tp, uint32_t index)
{
    if (index < tp->npieces - 1)
        ...
        fpos_t allbutlast = tp->piece_length * (tp->npieces - 1);
...
torrent_start(struct tlib *tl)
{
    ...
    memset(tp->sinfo,0,sizeof(tp->sinfo));
    tp->svrnum = 0;
    tp->tl = tl;
...
torrent_on_tick(struct torrent *tp)
{
    ...
    case T_STARTING:
	    ...
	    {
                tp->state = T_SEED;
		btpd_log(BTPD_L_BTPD, "Seeding for '%s'.\r\n",
			torrent_name(tp));
	    }
            else
                tp->state = T_LEECH;
...
void p2s_start(struct torrent *tp)
{
    ...
    struct peer *peer;
    int idx;
    for(idx = 0; idx < tp->svrnum; ++idx)
    {
	if(tp->sinfo[idx].done)
	    continue;
	url.host = tp->sinfo[idx].host;
	url.port = tp->sinfo[idx].port;
	url.uri = tp->sinfo[idx].uri;
	peer = peer_create_out_http(tp->net,&url,idx);
	if(peer)
	{
	    tp->sinfo[idx].done = 1;
	    peer->idx = idx;
