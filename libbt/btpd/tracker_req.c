...
entry_send(struct tr_tier *t, struct tr_entry *e, enum tr_event event)
{
	...
	ts.tv_sec = m_tnext_req - btpd_seconds;
	ts.tv_nsec = 0;
...
tr_result(struct tr_tier *t, struct tr_response *res)
{
	...
	{
	    ts.tv_sec = RETRY1_TIMEOUT;
	    ts.tv_nsec = 0;
            btpd_timer_add(&t->timer, &ts);
	}
        else
	{
	    ts.tv_sec = RETRY2_TIMEOUT;
	    ts.tv_nsec= 0;
            btpd_timer_add(&t->timer, &ts);
	}
        break;
    ...
    case TR_RES_OK:
	    ...
	    ts.tv_sec = t->interval > 0 ? t->interval : DEFAULT_INTERVAL;
	    ts.tv_nsec = 0;
            btpd_timer_add(&t->timer, &ts);
        }
        t->bad_conns = 0;
