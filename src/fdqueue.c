                                    ...
                                    apr_pool_t * pool_to_recycle)
{
    ...
    zevent_push_pool(queue_info, pool_to_recycle);
 ...
 */
#define zevent_queue_full(queue) ((queue)->nelts == (queue)->bounds)

/**
 * Detects when the fd_queue_t is empty. This utility function is expected
 * to be called from within critical sections, and is not threadsafe.
 */
#define zevent_queue_empty(queue) ((queue)->nelts == 0)

/**
 * Callback routine that is called to destroy this
 * fd_queue_t when its pool is destroyed.
 */
static apr_status_t zevent_queue_destroy(void *data)
{
                           ...
                           apr_pool_t * a)
{
    ...
    apr_pool_cleanup_register(a, queue, zevent_queue_destroy,
                              apr_pool_cleanup_null);
 ...
 *
 * precondition: zevent_queue_info_wait_for_idler has already been called
...
apr_status_t zevent_queue_term(fd_queue_t * queue)
{
    ...
    return zevent_queue_interrupt_all(queue);
}
