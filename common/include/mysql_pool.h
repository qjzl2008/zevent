...
/*setup init*/
MYSQL_DECLARE_NONSTD(apr_status_t) mysql_pool_init(apr_pool_t *pool, svr_cfg *s);
/* acquire a connection that MUST be explicitly closed.
 * Returns NULL on error
 */
MYSQL_DECLARE_NONSTD(MYSQL*) mysql_pool_acquire(apr_pool_t*, svr_cfg*);
/* release a connection acquired with ap_dbd_open */
MYSQL_DECLARE_NONSTD(void) mysql_pool_release(svr_cfg*, MYSQL*);

/*fini*/
MYSQL_DECLARE_NONSTD(apr_status_t) mysql_pool_fini(apr_pool_t *pool, svr_cfg *s);

/*sql op function*/
MYSQL_DECLARE_NONSTD(int) mysql_pool_query(apr_pool_t *pool,svr_cfg *s,
		const char *sql,
		apr_size_t len,
		MYSQL_RES **rs);

MYSQL_DECLARE_NONSTD(int) mysql_pool_exec(apr_pool_t *pool,svr_cfg *s,
		const char *sql,
		apr_size_t len,
		unsigned long *rows);
