 ...
 * than one RING_ENTRY field. Each RING_ENTRY has a corresponding
 * RING_HEAD declaration.
 *
 * @warning For strict C standards compliance you should put the RING_ENTRY
 ...
 * <pre>
 *      RING_HEAD(my_ring_t, my_element_t);
 ...
 * to the start of each element, not to the RING_ENTRY structure.
 *
 * <pre>
 *     +->+------+<-+  +->+------+<-+  +->+------+<-+
 ...
 * </pre>
 *
 * The RING_HEAD is nothing but a bare RING_ENTRY. The prev
 ...
 * name for its RING_ENTRY in each element, and has its own type
 * for its RING_HEAD.
 *
 * Note also that if the offset is non-zero (which is required if an
 * element has more than one RING_ENTRY), the unreality of the
 * sentinel may have bad implications on very perverse implementations
 * of C -- see the warning in RING_ENTRY.
 *
 ...
 *   empty rings... see RING_SPLICE_HEAD for one that does
 ...
 *   empty rings... see RING_SPLICE_TAIL for one that does
 ...
 *   empty rings... see RING_INSERT_HEAD for one that does
 ...
 *   empty rings... see RING_INSERT_TAIL for one that does
 ...
 * @param link The name of the RING_ENTRY in the element struct
 */
#define RING_PREPEND(h1, h2, elem, link) do {			\
	if (!RING_EMPTY((h2), elem, link)) {			\
	    RING_SPLICE_AFTER(RING_SENTINEL((h1), elem, link),	\
				  RING_FIRST((h2)),			\
				  RING_LAST((h2)), link);		\
	    RING_INIT((h2), elem, link);				\
	}								\
 ...
 * @param link The name of the RING_ENTRY in the element struct
 */
#define RING_FOREACH(ep, head, elem, link)                          \
    for (ep = RING_FIRST(head);                                     \
         ep != RING_SENTINEL(head, elem, link);                     \
 ...
 * @param link The name of the RING_ENTRY in the element struct
 */
#define RING_FOREACH_SAFE(ep1, ep2, head, elem, link)               \
    for (ep1 = RING_FIRST(head), ep2 = RING_NEXT(ep1, link);    \
         ep1 != RING_SENTINEL(head, elem, link);                    \
         ep1 = ep2, ep2 = RING_NEXT(ep1, link))
...
#define RING_CHECK_ELEM(ep, elem, link, msg) do {			\
	...
	do {								\
	    ...
	    fprintf(stderr, "\telem->next->prev %p\n",			\
		    RING_PREV(RING_NEXT(here, link), link));	\
	    fprintf(stderr, "\telem->prev->next %p\n",			\
		    RING_NEXT(RING_PREV(here, link), link));	\
	    if (RING_PREV(RING_NEXT(here, link), link) != here) { \
		fprintf(stderr, "\t*** elem->next->prev != elem\n");	\
		break;							\
	    }								\
	    if (RING_NEXT(RING_PREV(here, link), link) != here) { \
		fprintf(stderr, "\t*** elem->prev->next != elem\n");	\
		break;							\
	    }								\
	    here = RING_NEXT(here, link);				\
	} while (here != start);					\
	fprintf(stderr, "*** ring check end\n");			\
    } while (0)

#define RING_CHECK_CONSISTENCY(hp, elem, link)			\
	RING_CHECK_ELEM_CONSISTENCY(RING_SENTINEL(hp, elem, link),\
					elem, link)

#define RING_CHECK_ELEM_CONSISTENCY(ep, elem, link) do {		\
	struct elem *start = (ep);					\
	struct elem *here = start;					\
	do {								\
	    assert(RING_PREV(RING_NEXT(here, link), link) == here); \
	    assert(RING_NEXT(RING_PREV(here, link), link) == here); \
	    here = RING_NEXT(here, link);				\
	} while (here != start);					\
    } while (0)
 ...
 *   (This is a no-op unless RING_DEBUG is defined.)
 ...
 * an assertion if any inconsistency is found.  Same idea as RING_CHECK()
 ...
 * is found.  Same idea as RING_CHECK_ELEM() except that it's silent
 * if all is well.
 *   (This is a no-op unless RING_DEBUG is defined.)
 * @param ep   The element
 * @param elem The name of the element struct
 * @param link The name of the RING_ENTRY in the element struct
 */
#define RING_CHECK_ELEM_CONSISTENCY(ep, elem, link)
#endif
